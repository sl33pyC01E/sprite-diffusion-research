...
if (-not (Test-Path -LiteralPath $trainingReport -PathType Leaf)) {
    throw 'Six-way action-bundle refinement did not publish a complete report'
}
if (-not (Test-Path -LiteralPath $checkpoint -PathType Leaf)) {
    throw 'Six-way action-bundle refinement did not publish its EMA checkpoint'
}
foreach ($path in @($output, $stdout, $stderr)) {
    if (Test-Path -LiteralPath $path) {
        throw "Refusing to replace action-bundle evaluation artifact: $path"
    }
}
while ($true) {
...
if ($process.ExitCode -ne 0) {
    throw "Action-bundle evaluation exited with code $($process.ExitCode)"
}
if (-not (Test-Path -LiteralPath (Join-Path $output 'evaluation-report.json') -PathType Leaf)) {
    throw 'Action-bundle evaluation did not publish its report'
}
