...
$control = Join-Path $root `
    'data\experiments\mugen-six-action-dense-latent-motion-endpoint-control-from-step10000-v1-step3000'
$output = Join-Path $root `
    'data\experiments\mugen-six-action-dense-latent-motion-endpoint-action-from-step10000-v1-step3000'
$reports = Join-Path $root 'data\index\reports'
$stdout = Join-Path $reports `
    'mugen-six-action-dense-latent-motion-endpoint-action-from-step10000-v1-step3000.out.log'
$stderr = Join-Path $reports `
    'mugen-six-action-dense-latent-motion-endpoint-action-from-step10000-v1-step3000.err.log'

if (Get-Process -Id $WaitForProcessId -ErrorAction SilentlyContinue) {
    Wait-Process -Id $WaitForProcessId
}
if (-not (Test-Path -LiteralPath (Join-Path $control 'training-report.json') -PathType Leaf)) {
    throw 'Endpoint control did not publish a complete training report'
}
foreach ($path in @($output, $stdout, $stderr)) {
    if (Test-Path -LiteralPath $path) {
        throw "Refusing to replace endpoint-action artifact: $path"
    }
}
$actualParentSha256 = (
...
if ($actualParentSha256 -ne $parentSha256) {
    throw 'Endpoint-action parent checkpoint SHA-256 differs'
}
while ($true) {
...
if ($process.ExitCode -ne 0) {
    throw "Endpoint-action refinement exited with code $($process.ExitCode)"
}
if (-not (Test-Path -LiteralPath (Join-Path $output 'training-report.json') -PathType Leaf)) {
    throw 'Endpoint-action refinement did not publish its training report'
}
