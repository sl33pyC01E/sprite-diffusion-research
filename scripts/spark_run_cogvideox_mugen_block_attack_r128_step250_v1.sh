...
dataset="${root}/mugen-cogvideox-orange-fighter-block-attack-v4"
...
output="${root}/lora-orange-fighter-block-attack-r128-step250-v1"
log="${root}/lora-orange-fighter-block-attack-r128-step250-v1.log"

test "$(sha256sum "${model}/source-index.json" | cut -d' ' -f1)" = "98fbc592f23269a38d039d16f969844a9da073b56b24567772433d4b02e2f831"
test "$(sha256sum "${dataset}/manifest.json" | cut -d' ' -f1)" = "ff43db0badbf5b53c6eade31d8d943973947fa3be8627bdfd11ca5487d0777b6"
test "$(sha256sum "${trainer}" | cut -d' ' -f1)" = "78430e548e68f7f30e434abc877439f560f428686a00cf5e04ae471b3e6249e8"
if [[ -e "${output}" || -e "${log}" ]]; then
  echo "refusing to replace CogVideoX causal-gate output or log" >&2
  exit 1
fi
mkdir -p "${output}"
